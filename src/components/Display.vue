          ...
          <div v-if="item.type === 'nps'" class="nps">
            ...
            <el-row v-for="(row, rowIndex) in item.options" :key="rowIndex" class="nps_row">
              ...
              <el-col v-for="(lev, i) in item.level.split(',')" :key="i" :span="calcSpan(item.level)" class="nps_radio">
                <el-radio :label="i" v-model="item.npsValue[rowIndex]" class="nps_radio">&nbsp;</el-radio>
              </el-col>
            </el-row>
          </div>
        </el-card>
            ...
            if(data.code==0){
              ...
              if(parseInt(errId)) {
                ...
                for(let i=0;i<this.pageSize;i++) {
                  if(this.pageParams.pageArr[i].some(item => item.id == errId)) {
                    this.num = i + 1
                    break
                  }
                }
