      ...
      <el-col :span="18">
        <!--标签页-->
         <el-tabs type="border-card" v-model="activeName">
            <el-tab-pane label="问卷设计" name="wjsj" :disabled="nowSelect.status === 1">
              <!--内容区域-->
              <div class="content">
            ...
            <el-tab-pane label="回答统计" name="hdtj" :disabled="nowSelect.status===0">
...
//        nowTempData:[],//当前模板库页码
    ...
    watch: {
      nowSelect(newValue, oldValue) {
        if(newValue.status === 1) {
          this.activeName = 'hdtj'
        } else {
          this.activeName = 'wjsj'
        }
      }
    },
    computed:{
    ...
    methods:{
      ...
      //预览问卷
      previewWj(){
        let url=window.location.origin+"/preview/"+this.nowSelect.id;//问卷链接
        console.log(url);
        window.open(url);
      },
      //编辑问卷
